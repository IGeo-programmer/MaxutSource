...
<div id = "content-size">
		...
		<?
			$GetTours = mysql_query("SELECT * FROM tours ORDER BY id DESC") or die(mysql_error());
			$GetToursRow = mysql_fetch_array($GetTours);
			do
			{
			echo '<article class="col-lg-3 col-md-3 col-sm-3 col-xs-6 col-xxs-12 animate-box">
				<figure>
					<a href="tour.php?id='.$GetToursRow[id].'"><img  src="'.$GetToursRow[main_pic].'" alt="Image" class="img-responsive"></a>
					
					
					<span class="fh5co-meta"><a href="tour.php?id='.$GetToursRow[id].'">'.$GetToursRow[name_geo].'</a></span>
				</figure>
				
				
			</article>';
			}
			while($GetToursRow = mysql_fetch_array($GetTours));
			?>
		</div>
	</div>
